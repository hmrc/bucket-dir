...
class BucketDirGenerator:
    ...
    def render_index_document(self, folder, folders, contents):
        ...
        matching_contents = []
        if folder == "":
            matching_contents.extend([content for content in contents if re.match("^[a-z0-9A-Z!\-_.*'()\s]+\/?$", content["Key"])])
        else:
            matching_contents.extend([content for content in contents if re.match(f"^{folder}\/[a-z0-9A-Z!\-_.*'()\s]+/?$", content["Key"])])
        for matching_content in matching_contents:
            name = matching_content["Key"].split('/')[-1] or f"{matching_content['Key'].split('/')[-2]}/"
            if name == "index.html":
                continue
        ...
        matching_subfolders = [subfolder for subfolder in folders if re.match(f"^{folder}\/[a-z0-9A-Z!\-_.*'()]+\/?$", subfolder)]
        ...
        for index_item in index_items:
            index_item["padding"] = " ".ljust(longest_name_length - len(index_item["name"]) + column_padding)
            index_item["modified"] = index_item["modified"].ljust(longest_modified_length + column_padding)
        ...
        return template.render(full_path=f"{folder}/", table_headers=table_headers, index_items=index_items, not_root=not_root)
...
def run_cli():
    argument_parser = argparse.ArgumentParser(description="Generate directory listings for S3 statically hosted content.")
