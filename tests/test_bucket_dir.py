...
def index_created_correctly(items, page_name, root_index=False):
    regular_expressions = [
        ...
        '<address style="font-size:small;">Generated by magic<\/address>',
    ]
    if not root_index:
        regular_expressions.append('<a href="\.\.\/">\.\.\/<\/a><\/br>')
    for item in items:
        regular_expressions.append(
            f"<a href=\"{item['name']}\">{item['name']}<\/a>\s+{item['last_modified']}\s+{item['size']}\\n"
        )
...
# TODO: Confirm preexisting index.html files are ignored
# <Contents>
